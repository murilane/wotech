...
Shop.java
```java
// itemservice should be in shop class to be able to use them
// get items from cart
...
// item manager functionality
  //add item
  //remove item
  //update item
  //bonus: item availability
  ...
  private ArrayList<Item> cart = new ArrayList<Item>();
    ...
    public void addItemToCart(Item item){
      //items list would contain the amount of available items
      //before we add item to the cart, we check whether it is available
      cart.add(item);
    }
  public int checkout(){
    int sum = 0;
...
public void removeItem(int id){
  for(var item : items){
    if (item.getId() == id){
      ...
      //return the function, stops the function
      // can use break, but better to use return
      // return for a void method is like break for a loop
    }
  }
}
public void updateItem(int id, String name, int cost){
  ...
  for (var item : items){
    if(item.getId() == id){
      ...
      item.setCost(cost);
      return;
      // can also do with streams/ifpresent
    }
  }
  }
}
